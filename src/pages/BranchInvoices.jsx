...
import {
  watchBranchInvoices, BRANCH_INVOICE_STATUS,
  agingFor, effectiveBranchStatus,
...
export default function BranchInvoices() {
  ...
  const decorated = useMemo(() => rows.map((r) => ({
    ...r,
    _eff: effectiveBranchStatus(r, now),
  ...
  const counts = useMemo(() => {
    const c = {
      [BRANCH_INVOICE_STATUS.OPEN]: 0,
      OVERDUE: 0,
      [BRANCH_INVOICE_STATUS.PAID]: 0,
      [BRANCH_INVOICE_STATUS.VOID]: 0,
      ALL: rows.length,
    }
  ...
  const buckets = useMemo(() => {
    ...
    for (const r of decorated) {
      if (r.status !== BRANCH_INVOICE_STATUS.OPEN) continue
  ...
  const openTotal = useMemo(() =>
    decorated
      .filter((r) => r.status === BRANCH_INVOICE_STATUS.OPEN)
          ...
          <div className="overflow-x-auto">
            <table className="min-w-full text-sm whitespace-nowrap">
              ...
              <tbody className="divide-y">
                {filtered.length === 0 && (
                  <tr><td colSpan={9} className="px-4 py-8 text-center text-gray-400">No branch invoices.</td></tr>
                )}
                {filtered.map((r) => {
                  ...
                  return (
                    <tr key={r.id || r.code} className="hover:bg-gray-50">
                      ...
                      <td className="px-4 py-2 text-xs"><DueCell invoice={r} /></td>
                      <td className="px-4 py-2">
                      ...
                      <td className="px-4 py-2 text-right">
                        {r.status === BRANCH_INVOICE_STATUS.PAID
                          ? <span className="text-green-700 text-xs">Paid</span>
                          : r.status === BRANCH_INVOICE_STATUS.VOID
  ...
  return (
    ...
    >
      ...
      <div className="flex items-center justify-between gap-2 mt-2 text-[11px] text-gray-400">
        ...
        {r.status === BRANCH_INVOICE_STATUS.OPEN ? (
...
function DueCell({ invoice }) {
  ...
  if (invoice.status !== BRANCH_INVOICE_STATUS.OPEN) {
